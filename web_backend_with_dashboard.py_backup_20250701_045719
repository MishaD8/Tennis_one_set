#!/usr/bin/env python3
"""
🎾 Tennis Backend with Integrated Dashboard
Backend с встроенным dashboard на главной странице
"""
...
<head>
    ...
    <title>🎾 Tennis Analytics Dashboard</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="controls">
            ...
            <button class="btn" onclick="testPrediction()">🔮 Test Match Prediction</button>
            <button class="btn" onclick="loadMatches()">📊 Load Match Data</button>
            <button class="btn" onclick="refreshData()">🔄 Refresh System</button>
        </div>

        <div id="matches-container" class="matches-container">
            <div class="loading">📊 Loading tennis analytics...</div>
        </div>
    </div>
        ...
        async function loadStats() {
            try {
                ...
                if (data.success && data.stats) {
                    ...
                    document.getElementById('api-status').textContent = '⚠️ Limited';
                }
            } catch (error) {
        ...
        async function loadMatches() {
            ...
            try {
                ...
                if (data.success && data.matches && data.matches.length > 0) {
                            ...
                            <div class="match-header">
                                <div>
                                    ...
                                    <div style="color: #7f8c8d; margin-top: 5px;">
                                        🏟️ ${match.tournament} • ${match.surface} • ${match.date} ${match.time}
                                    </div>
                                </div>
                    ...
                    container.innerHTML = '<div class="loading">📊 No matches available. Using demo data.</div>';
                    showDemoMatches();
                }
            } catch (error) {
                console.error('Matches error:', error);
                container.innerHTML = '<div class="loading">❌ Error loading matches. Showing demo data.</div>';
                showDemoMatches();
            }
        }
...
@app.route('/')
def dashboard():
    """Главная страница с встроенным dashboard"""
...
if __name__ == '__main__':
    print("🎾 TENNIS PREDICTION BACKEND WITH INTEGRATED DASHBOARD")
    print("=" * 70)
